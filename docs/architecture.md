...
The API uses `/api` as its prefix. Health, scan status, albums, tracks, cached artwork, track media, and queue mutations are exposed. Media requests accept only a catalogue track ID. The server resolves its stored relative path beneath the configured library root, rejects escapes (including resolving symbolic links), and supports one HTTP byte range for browser seeking.
...
- Spectrum, Golden Ratio and Concentric Squares share that single stream. A frontend registry mounts only the selected canvas renderer and logarithmically aggregates the existing analyser centres to each renderer's required band count. Per-renderer sensitivity is a browser-local display adjustment; it adds no FFT work or PCM transport.
...
- Internet Radio uses that same Bluetooth mutual-exclusion gate and persistent `HTMLAudioElement`. Its centralized, small HTTPS station catalogue is frontend-owned; selecting another tile replaces only the element source, Stop restores any current local queue item paused, and bounded reconnect attempts remain presentation failures rather than queue or backend failures. A separate bounded backend request reads only one ICY metadata block for the selected curated stream and returns compact, sanitised now-playing text; it never proxies audio or gates playback. The final DAC monitor therefore observes Radio without a second player, PCM transport, or routing change.
