...
def test_update_check_current_and_newer_release() -> None:
    ...
    newer = UpdateService(Settings(_env_file=None), Source(release_info("0.6.18")))
    assert newer.check()
...
def test_release_without_required_assets_is_visible_but_not_installable(tmp_path: Path) -> None:
    ...
    service = UpdateService(settings, Source(release_info("0.6.18", assets=False)))
    service.check()
    wait_until(lambda: not service.status()["checking"])
...
def test_old_terminal_outcome_does_not_hide_a_later_release(tmp_path: Path) -> None:
    ...
    service = UpdateService(settings, Source(release_info("0.6.18")))
    service.check()
    wait_until(lambda: not service.status()["checking"])

    assert service.status()["outcome"] is None
    assert service.status()["latest_version"] == "0.6.18"
    assert service.status()["message"] == "Version 0.6.18 is available."
...
def helper_config(tmp_path: Path) -> HelperConfig:
    return HelperConfig(
        ...
        release_root=active_root(tmp_path, "0.6.17"),
...
def write_request(config: HelperConfig, version="0.6.18") -> None:
...
def test_helper_persists_success_and_rollback_outcomes(tmp_path: Path, monkeypatch) -> None:
    ...
    archive, manifest = make_release(tmp_path / "package", version="0.6.18")

    success_config = helper_config(tmp_path / "success")
    write_request(success_config)
    success_controller = FakeController({"0.6.18"})
    ...
    rollback_controller = FakeController({"0.6.17"})
    ...
    assert "restored to 0.6.17" in rollback_status["message"]
    assert (rollback_config.release_root / "current-version").read_text().strip() == "0.6.17"
